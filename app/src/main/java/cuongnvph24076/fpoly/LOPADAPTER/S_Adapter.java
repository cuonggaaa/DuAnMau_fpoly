package cuongnvph24076.fpoly.LOPADAPTER;
...
import cuongnvph24076.fpoly.QuanlyvsThongke.Quanlysach.SachViewModel;
import cuongnvph24076.fpoly.R;
import cuongnvph24076.fpoly.SPINERADAPTER.LoaiSachSpiner;
...
public class S_Adapter extends RecyclerView.Adapter<S_Adapter.SachHoder> implements Filterable {
    ...
    public void onBindViewHolder(@NonNull @NotNull SachHoder holder, int position) {
        ...
        if (sach == null) {
            ...
            holder.textSoLuong.setText("Số lượng: " + sach.getSoLuong());
        }
        ...
        holder.textSoLuong.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                TextView textBookName = dialog.findViewById(R.id.textBookName);
                EditText input = dialog.findViewById(R.id.inputSoLuongUpdate);
                Button button = dialog.findViewById(R.id.buttonUpdateSoLuong);

                input.setText(sach.getSoLuong() + "");
                textBookName.setText(sach.getTens());

                button.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        String soLuongInput = input.getText().toString().trim();
                        if (soLuongInput.isEmpty()) {
                            Toast.makeText(context, "Vui long dien day du thong tin!", Toast.LENGTH_SHORT).show();
                            return;
                        }
                        ...
                        dialog.cancel();
                        Toast.makeText(context, "Updat thanh cong!", Toast.LENGTH_SHORT).show();
                    }
                });
            }
        });
    ...
    public class SachHoder extends RecyclerView.ViewHolder {
        TextView tv_ms, tv_mls, tv_tens, tv_gias, tv_tacgia, textSoLuong;
